 ...
 *
 * This node provides a service to add two integers. The service takes two
 * integer inputs and returns their sum.
 */
...
class AddTwoIntsService : public rclcpp::Node {
 public:
  ...
  AddTwoIntsService() : Node("add_two_ints_service") {
    service_ = this->create_service<example_interfaces::srv::AddTwoInts>(
        "add_two_ints",
        std::bind(&AddTwoIntsService::handle_add_two_ints, this,
                  std::placeholders::_1, std::placeholders::_2));
    RCLCPP_INFO(this->get_logger(),
                "Service 'add_two_ints' is ready to add two numbers.");
  }
 ...
 private:
  ...
  void handle_add_two_ints(
    ...
    RCLCPP_DEBUG(this->get_logger(), "Received request: a=%ld, b=%ld",
                 request->a, request->b);
    // setting the result to send to the client
    response->sum = request->a + request->b;
    RCLCPP_INFO(this->get_logger(), "Sending response: %ld", response->sum);
    RCLCPP_WARN(this->get_logger(),
                "Warning: ensure inputs are within valid range.");
    RCLCPP_ERROR(this->get_logger(),
                 "Simulating an error if inputs were invalid (hypothetical).");
